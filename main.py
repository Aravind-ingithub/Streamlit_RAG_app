...
# Modern Google & Groq LangChain integrations
...
st.title("📚 Groq-Powered Custom Knowledge Base Chatbot")
st.write("Upload a PDF document, build the index, and chat with your data at warp speed!")
...
GROQ_API_KEY = st.secrets["GROQ_API_KEY"]  # New Secret Key!
...
def process_and_index_pdf(uploaded_file):
    ...
    # Dense Vector Setup (Keep Gemini for Embeddings)
    ...
    # Initialize Groq Llama Model Core
        ...
        if st.button("🏗️ Build Knowledge Base"):
            with st.spinner("Processing document layout..."):
                ...
                if compiled_chain:
                    ...
                    # Clear past records if a brand new knowledge layout is loaded
                    st.session_state["chat_history"] = [] 
